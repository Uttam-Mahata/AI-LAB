...
class CityGrid:
    """
    Represents the 3D city as a grid[z][x][y] of cell strings.

    Attributes:
        rows, cols, levels : grid dimensions
        grid               : list[list[list[str]]] indexed [z][x][y]
        start              : (x, y, z) of 'S'
        base               : (x, y, z) of 'B'
        goals              : dict  'G1' → (x, y, z), …
        recharge_stations  : list of (x, y, z) for all 'R' cells
    """
    ...
    def get_cell(self, x, y, z):
        """Return cell string at (x,y,z), or None if out of bounds."""
    ...
    def terrain_cost(cell):
        """Terrain energy cost for entering a cell."""
        return 3.0 if cell == FIRE else 1.0

    @staticmethod
    def is_passable(cell):
        """Return True if a drone can fly into this cell."""
        return cell is not None and cell != OBSTACLE

    def move_cost(self, dest_cell, dz):
        """
        Total energy cost to move into dest_cell.

            Horizontal (dz = 0)  : terrain_cost(dest_cell)
            Ascending  (dz = +1) : terrain_cost(dest_cell) + 2  (fight gravity)
            Descending (dz = -1) : 1                             (gravity assist)
        """
        if dz > 0:
    ...
    def get_neighbors(self, pos):
        """
        Return list of (neighbor_pos, move_cost) for all valid moves from pos.
        """
        x, y, z = pos
        result = []
...
def heuristic(a, b):
    ...
    Admissible: since each step costs at least 1 unit, h(n) ≤ actual cost.
...
def astar_3d(city_grid, start, goal):
    """
    A* pathfinding on a 3D city grid.

    Args:
        city_grid : CityGrid instance
        start     : (x, y, z) start position
        goal      : (x, y, z) goal position

    Returns:
        (path, cost)  where path is list[(x,y,z)] from start to goal inclusive
...
def _reconstruct_path(came_from, current):
    """Backtrack came_from dict to build the full path."""
    path = [current]
...
def _greedy_goal_order(start, goals_dict):
    """
    Order survivor goals using the greedy nearest-neighbour heuristic.

    Starting from `start`, repeatedly pick the closest unvisited survivor
    (by Euclidean distance) until all are ordered.

    Returns list of (label, pos) tuples in visit order.
    """
...
def _find_nearest_recharge(city_grid, current_pos):
    """
    Find the nearest reachable recharging station via A*.

    Returns:
        (pos, path, cost)  for the closest reachable station
        (None, None, inf)  if none is reachable
    """
...
def plan_mission(city_grid, initial_energy, recharge_penalty):
    """
    Plan and execute the full rescue mission.

    Mission sequence: S → G1 → G2 → … → Gn → B
    Goals are visited in greedy nearest-neighbour order.

    If energy is insufficient for the next segment, the drone is diverted to
    the nearest recharging station (R).  After recharging, energy is restored
    to (initial_energy - recharge_penalty).

    Args:
        city_grid        : CityGrid instance
        initial_energy   : starting (and post-recharge) energy level
        recharge_penalty : energy deducted as a time penalty when recharging

    Returns:
        dict with keys:
            success          : bool
            full_path        : list[(x,y,z)]  complete flight path
            total_cost       : float          total energy consumed
            energy_remaining : float
            segments         : list[dict]     per-segment details
            log              : list[str]      human-readable mission log
    """
    log      = []
    segments = []
...
def visualize_grid(city_grid, path_positions):
    """
    Render a layer-by-layer ASCII view of the 3D grid with the path marked.

    Legend:
        S  = Start      B  = Base       R  = Recharge
        Gn = Survivor   F  = Fire zone  #  = Obstacle
        *  = Path step  .  = Free space
    """
    lines = []
